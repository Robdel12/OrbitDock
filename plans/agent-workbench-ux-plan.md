...
This plan depended on the server v2 refactor (functional core, imperative shell — see `plans/completed/server-architecture-v2.md`). All server v2 work is complete and shipped.
...
## Phase 5: Approval Oversight v2 — COMPLETE
...
**Status**: ✅ Complete. All core scope items implemented.

### Scope
- [x] **Diff preview on patch approvals**: `ApprovalDiffPreview.swift` renders `ServerApprovalRequest.diff` inline using `DiffModel.parse()`. Shows file sections with per-file +N/-N stats. Collapsible with toggle header.
- [x] **Risk cues**: `ApprovalRisk.swift` enum with three tiers (low/normal/high). Detects destructive patterns (`rm -rf`, `git push --force`, `DROP TABLE`, `sudo`, etc.). Color-coded stripe, icon, and "DESTRUCTIVE" badge on high-risk approvals.
- [x] **Keyboard-first triage**: `y` (approve once), `Y` (approve for session), `!` (always allow), `n` (deny), `N` (deny & stop/abort). Key hints rendered on all buttons. Uses `.onKeyPress(phases: .down)`.
- [x] **Approval card redesign**: Full redesign using `Spacing`, `TypeScale`, `Radius`, `OpacityTier` design tokens. Risk stripe, tool details section, inline diff preview, primary/secondary action buttons with hover states.
- [x] **Approval history in rail**: `CodexApprovalHistoryView` moved into a `CollapsibleSection` in `CodexTurnSidebar`. Shows session/global scope toggle, decision labels, relative timestamps.

### New files created
- `Views/Codex/ApprovalDiffPreview.swift` — Inline diff rendering for approval cards
...
- [x] Dangerous commands are visually distinct from routine approvals.
- [x] User can approve/deny entirely via keyboard without touching the mouse.
- [x] Key hints visible on approval buttons so keybindings are discoverable.
- [x] Approval history accessible from the capability rail.
...
- [x] Phase 5: Approval Oversight v2 (diff previews, risk cues, keyboard-first triage, design token alignment).
