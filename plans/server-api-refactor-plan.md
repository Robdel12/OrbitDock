...
- `Slice 3Z`: review the remaining `control_mode` compatibility fallbacks that may now be shrinkable after startup normalization and the recent projection/read-path cleanup. Start with `startup_recovery.rs`, `session_reads/ownership_reads.rs`, `runtime/session_queries.rs`, and `transport/http/server_meta/usage.rs`.
