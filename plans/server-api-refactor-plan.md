...
- `Slice 3F`: complete. The first low-risk `dead_code` prune lane is now landed: the unused `reset_data_dir()` helper is gone, the unused `get_session_list_items()` registry helper is gone, the unused `session_id` field is gone from `ConnectorCleanupGuard`, stale `#[allow(dead_code)]` suppressions from the test extraction pass have started disappearing, and the unreferenced `SessionActorHandle` convenience methods `try_send`, `snapshot_swap`, and `command_tx` are gone too.
