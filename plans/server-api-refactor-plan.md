...
- [ ] Recut `startup_recovery.rs` around one explicit ownership model for restored direct/passive sessions.
  In progress: startup restore and by-id hydration now share one `load_restored_session_supplement(...)` path for config/usage/diff/summary/approval metadata materialization, and startup recovery now carries `summary`, `claude_sdk_session_id`, and `end_reason` in its primary `ActiveSessionRow` projection instead of re-querying them inside the restore loop, while the startup repair SQL and row-loading policy remain caller-owned until the next cut.
...
- [ ] Update native mirror types and decoding so they match the narrowed protocol surface in the same slice.
  In progress: the dead native `ServerConversationSnapshotPayload` and `ServerSessionSummary` mirrors are removed, lifecycle response DTOs only decode live fields, `ServerConversationApprovalRow` no longer carries the unused `request` mirror, and `ServerSessionListItem` no longer mirrors the stale `display_title_sort_key` / `display_search_text` fields now that root-shell search/sort derive locally.
