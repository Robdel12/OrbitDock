...
- `Slice 3Q`: review the remaining Claude direct/passive shadow-session preservation and cleanup branches, focusing on duplicated shadow-row suppression/cleanup guards rather than the still-live passive Claude session flow itself.
- `Slice 3S`: decide whether the persistence-layer Claude shadow guards (`preserve_direct_owned_claude_shadow` and friends) are still needed once hook-layer routing and startup cleanup already suppress direct-owned shadow rows.
