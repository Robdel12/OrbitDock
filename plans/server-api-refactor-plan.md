...
- `Slice 3P`: decide whether the `mixed_legacy` usage repair path in `infrastructure/persistence/usage.rs` still supports real persisted data we intend to carry forward, or whether it should be deleted instead of preserved as compatibility ballast.
