...
- `Slice 3Q`: review the remaining Claude direct/passive shadow-session preservation and cleanup branches now that the lower-risk dead helper lane is exhausted.
- `Slice 3R`: audit the remaining persistence/session-read compatibility checks after the recent slice sequence and cut any stale fallback logic that is no longer reachable on supported databases.
