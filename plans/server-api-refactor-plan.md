...
- `Slice 3S`: if we touch the Claude shadow lane next, narrow it to the startup restore belt-and-suspenders path in `startup_recovery.rs` rather than removing the persistence-layer guard. The only plausible small delete there is the restore select-side duplicate exclusion after the earlier startup `UPDATE` has already ended the same shadow rows.
