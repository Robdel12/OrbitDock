...
- `Slice 3Y`: review the remaining domain-session compatibility shims next: the `session.rs` facets re-export surface that still fronts `SessionIdentity`, `SessionConfig`, `SessionDisplay`, `SessionEnvironment`, and `SessionTimestamps`.
