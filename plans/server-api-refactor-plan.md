...
- `Slice 3X`: review the domain-session compatibility shims next: `SessionConfigPatch` as a pure alias of `SessionConfig`, plus the `session.rs` facets re-export surface that still fronts `SessionIdentity`, `SessionConfig`, `SessionDisplay`, `SessionEnvironment`, and `SessionTimestamps`.
