...
fn execute_command_by_family(
  ...
  match cmd {
    ...
    PersistCommand::SetSessionConfig {
      ...
      model,
      effort,
      codex_config_mode.map(Some),
      codex_config_profile.map(Some),
      codex_model_provider.map(Some),
      codex_config_source.map(Some),
      codex_config_overrides_json.map(Some),
    )?,
    ...
    PersistCommand::ReviewCommentCreate {
      ...
      turn_id,
      file_path,
      Some(line_start as i64),
      line_end.map(|value| value as i64),
      body,
      tag,
    )?,
    ...
    PersistCommand::MissionIssueUpdateState {
      ...
      orchestration_state,
      session_id.map(Some),
      workspace_id.map(Some),
      attempt.map(Some),
      last_error.map(Some),
      retry_due_at.map(Some),
      started_at.map(Some),
      completed_at.map(Some),
    )?,
...
/// Legacy monolithic executor kept temporarily while the new family-dispatched
/// entrypoint settles. Once the refactor is fully bedded in, this should be
/// deleted rather than maintained in parallel.
#[allow(dead_code)]
fn execute_command_legacy(conn: &Connection, cmd: PersistCommand) -> Result<(), rusqlite::Error> {
  match cmd {
...
fn extract_row_content(row: &ConversationRow) -> Option<String> {
  match row {
    ...
    ConversationRow::CommandExecution(command) => Some(command.command.clone()),
