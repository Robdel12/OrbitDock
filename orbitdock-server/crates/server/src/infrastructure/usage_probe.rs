...
async fn send_json_rpc(
  ...
  let mut bytes = serde_json::to_vec(&payload)
    .map_err(|err| UsageProbeError::RequestFailed(format!("Invalid JSON-RPC payload: {err}")))?;
  bytes.push(b'\n');
  ...
  stdin
    .flush()
    .await
    .map_err(|err| UsageProbeError::RequestFailed(format!("Failed to flush codex stdin: {err}")))?;
  Ok(())
}
...
async fn read_json_rpc_response(
  ...
  loop {
    ...
    let value: Value = serde_json::from_str(trimmed)
      .map_err(|err| UsageProbeError::RequestFailed(format!("Invalid codex response: {err}")))?;
