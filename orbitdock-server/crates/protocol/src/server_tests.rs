...
fn roundtrip_correlated_utility_responses() {
  ...
  match serde_json::from_str::<ServerMessage>(&codex_usage_json).expect("deserialize codex usage") {
  ...
  match serde_json::from_str::<ServerMessage>(&claude_usage_json).expect("deserialize claude usage")
  {
