...
fn roundtrip_codex_login_chatgpt_start() {
  ...
  let parsed: ClientMessage = serde_json::from_str(json).expect("parse codex_login_chatgpt_start");
  match &parsed {
...
fn roundtrip_codex_login_chatgpt_cancel() {
  ...
  let parsed: ClientMessage = serde_json::from_str(json).expect("parse codex_login_chatgpt_cancel");
  match &parsed {
...
fn roundtrip_correlated_utility_requests() {
  ...
  match serde_json::from_str::<ClientMessage>(&claude_usage_json).expect("deserialize claude usage")
  {
