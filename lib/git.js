...
export const getMainBranch = (cwd) => {
  try {
    ...
    const branches = execGit('git branch -l main master', cwd)
...
export const getRepoRoot = (cwd) => {
  try {
    ...
    let root = execGit('git rev-parse --show-toplevel', cwd)

    // Check if this is a worktree by looking at git-common-dir
    const gitCommonDir = execGit('git rev-parse --git-common-dir', cwd)
...
export const getWorktreeRoot = (cwd) => {
  try {
    return execGit('git rev-parse --show-toplevel', cwd)
  } catch {
    return null
  }
}
...
export const getCommitCount = (cwd) => {
  try {
    ...
    const count = execGit(`git rev-list --count ${main}..HEAD`, cwd)
...
export const getDiffStats = (cwd) => {
  try {
    const main = getMainBranch(cwd)
    const stats = execGit(`git diff --shortstat ${main}...HEAD`, cwd)
