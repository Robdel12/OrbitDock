...
struct ServerRolePlan: Sendable {
  let enabledEndpointIds: [UUID]
  let primaryEndpointId: UUID?

}

extension ServerRolePlan: Equatable {}
...
struct ServerRolePort: Sendable {
  let endpointId: UUID
  let client: ServerRoleClient
}

actor ServerRoleCoordinator {
  private var appliedPrimaryClaimAssignments: [UUID: Bool] = [:]
  private var pendingPrimaryClaimPlan: ServerRolePlan?
  private var pendingPrimaryClaimPorts: [UUID: ServerRolePort] = [:]
  ...
  func submitPrimaryClaimPlan(
    _ plan: ServerRolePlan,
    ports: [ServerRolePort],
