...
struct SessionsClient: Sendable {
  ...
  func resumeSession(_ sessionId: String) async throws -> ResumeSessionResponse {
    try await http.post(
      "/api/sessions/\(requestBuilder.encodePathComponent(sessionId))/lifecycle/resume",
  ...
  func takeoverSession(_ sessionId: String, request: TakeoverRequest) async throws -> TakeoverResponse {
    try await http.post(
      "/api/sessions/\(requestBuilder.encodePathComponent(sessionId))/lifecycle/takeover",
      body: request
    )
  }

  func endSession(_ sessionId: String) async throws -> ServerAcceptedResponse {
    try await http.post(
      "/api/sessions/\(requestBuilder.encodePathComponent(sessionId))/lifecycle/end",
      body: ServerEmptyBody()
    )
  }

  func renameSession(_ sessionId: String, name: String?) async throws -> ServerAcceptedResponse {
    struct Body: Encodable { let name: String? }
    return try await http.request(
      path: "/api/sessions/\(requestBuilder.encodePathComponent(sessionId))/detail/name",
      method: "PATCH",
      body: Body(name: name)
    )
  }

  func setSummary(_ sessionId: String, summary: String) async throws -> ServerAcceptedResponse {
    struct Body: Encodable { let summary: String }
    return try await http.request(
      path: "/api/sessions/\(requestBuilder.encodePathComponent(sessionId))/detail/summary",
      method: "PATCH",
  ...
  func updateSessionConfig(
    ...
    try await http.request(
      path: "/api/sessions/\(requestBuilder.encodePathComponent(sessionId))/detail/config",
      method: "PATCH",
      body: config
    )
  }
  ...
  func forkSession(_ sourceSessionId: String, request: ForkRequest) async throws -> ForkResponse {
    try await http.post(
      "/api/sessions/\(requestBuilder.encodePathComponent(sourceSessionId))/lifecycle/fork",
      body: request
    )
  }
  ...
  func forkSessionToWorktree(
    ...
    try await http.post(
      "/api/sessions/\(requestBuilder.encodePathComponent(sourceSessionId))/lifecycle/fork/worktree",
      body: request
    )
  }
  ...
  func forkSessionToExistingWorktree(
    ...
    try await http.post(
      "/api/sessions/\(requestBuilder.encodePathComponent(sourceSessionId))/lifecycle/fork/existing-worktree",
      body: request
    )
  }
  ...
  func getSubagentTools(sessionId: String, subagentId: String) async throws -> [ServerSubagentTool] {
    ...
    let response: Response = try await http.get(
      subagentPath(sessionId: sessionId, subagentId: subagentId, suffix: "tools")
    )
  ...
  func markSessionRead(_ sessionId: String) async throws -> UInt64 {
    ...
    let response: Response = try await http.post(
      "/api/sessions/\(requestBuilder.encodePathComponent(sessionId))/detail/read",
      body: ServerEmptyBody()
    )
