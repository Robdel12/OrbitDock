...
@MainActor
struct ServerSessionAPILocalConversationNamingTests {
  @Test func sendMessageSetsSummaryFromAuthoritativeFirstPromptOnlyOnce() async throws {
    ...
    let harness = try makeSession(loader: { request in try await fixture.loader(request) })
    let session = harness.session
    session.api.localNamingAvailabilityOverride = .available
    session.api.localTitleGenerator = { context in
      await fixture.generateTitle(for: context)
    }

    _ = try await session.api.sendMessage(content: "Investigate Apple Foundation title resets")
    await fixture.waitForSummaryRequestCount(1)

    _ = try await session.api.sendMessage(content: "Give me a progress update")
  ...
  @Test func fetchSessionDetailDedupesConcurrentBootstrapRequests() async throws {
    let fixture = LocalNamingHTTPFixture(
      sessionState: makeSessionStateObject(
        firstPrompt: "Investigate Apple Foundation title resets"
      ),
      generatedTitle: nil,
      pauseFirstDetailRequest: true
    )
    let harness = try makeSession(loader: { request in try await fixture.loader(request) })
    let session = harness.session

    async let first: ServerSessionDetailSnapshotPayload = session.api.fetchSessionDetail()
    await fixture.waitForDetailRequestCount(1)
    async let second: ServerSessionDetailSnapshotPayload = session.api.fetchSessionDetail()
    await Task.yield()

    #expect(await fixture.detailRequestCount == 1)

    await fixture.resumeFirstDetailResponse()
    let firstPayload = try await first
    let secondPayload = try await second

    #expect(firstPayload.session.id == "session-1")
    #expect(secondPayload.session.id == "session-1")
    #expect(await fixture.detailRequestCount == 1)
  }
  ...
  private func makeSession(loader: @escaping ServerClients.DataLoader) throws -> SessionHarness {
    ...
    return SessionHarness(runtime: runtime, sessionId: "session-1")
  }
...
private final class SessionHarness {
  ...
  init(runtime: ServerEndpointRuntime, sessionId: String) {
...
private actor LocalNamingHTTPFixture {
  ...
  private let pauseFirstDetailRequest: Bool
  ...
  private var detailResponseContinuation: CheckedContinuation<Void, Never>?
  private var hasPausedFirstDetailRequest = false
  ...
  init(
    sessionState: [String: Any],
    generatedTitle: String?,
    pauseFirstDetailRequest: Bool = false
  ) {
    self.sessionState = sessionState
    self.generatedTitle = generatedTitle
    self.pauseFirstDetailRequest = pauseFirstDetailRequest
  }
  ...
  func resumeFirstDetailResponse() {
    detailResponseContinuation?.resume()
    detailResponseContinuation = nil
  }
  ...
  func loader(_ request: URLRequest) async throws -> (Data, URLResponse) {
    ...
    switch (method, path) {
      ...
      case ("GET", "/api/sessions/session-1/detail"):
        ...
        if pauseFirstDetailRequest && !hasPausedFirstDetailRequest {
          hasPausedFirstDetailRequest = true
          await withCheckedContinuation { continuation in
            detailResponseContinuation = continuation
          }
        }
      ...
      case ("PATCH", "/api/sessions/session-1/detail/summary"):
