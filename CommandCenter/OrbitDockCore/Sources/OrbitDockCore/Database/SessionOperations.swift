...
public extension CLIDatabase {
  ...
  func clearActiveSubagent(id sessionId: String) throws {
    if shouldIgnoreHookWrite(sessionId: sessionId) { return }
    try connection.run(
      """
      UPDATE sessions SET
  ...
  func incrementCompactCount(id sessionId: String) throws {
    if shouldIgnoreHookWrite(sessionId: sessionId) { return }
    try connection.run(
      """
      UPDATE sessions SET
  ...
  func clearPendingFields(id sessionId: String) throws {
    if shouldIgnoreHookWrite(sessionId: sessionId) { return }
    try connection.run(
      """
      UPDATE sessions SET
  ...
  /// End a session
  func endSession(id sessionId: String, reason: String?) throws {
    if shouldIgnoreHookWrite(sessionId: sessionId) { return }
    let now = Self.formatDate()
    try connection.run(
      """
  ...
  func incrementPromptCount(id sessionId: String) throws {
    if shouldIgnoreHookWrite(sessionId: sessionId) { return }
    try connection.run(
      """
      UPDATE sessions SET
  ...
  func incrementToolCount(id sessionId: String) throws {
    if shouldIgnoreHookWrite(sessionId: sessionId) { return }
    try connection.run(
      """
      UPDATE sessions SET
  ...
  /// Get session by ID
  func getSession(id sessionId: String) -> SessionRow? {
  ...
  func cleanupStaleSessions(terminalId: String, currentSessionId: String) throws -> Int {
    let now = Self.formatDate()
