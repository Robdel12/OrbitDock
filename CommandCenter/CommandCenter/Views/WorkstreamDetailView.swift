...
struct WorkstreamDetailView: View {
    ...
    private func noteTypeColor(_ type: WorkstreamNote.NoteType) -> Color {
        switch type {
        case .note: return Color.textSecondary
        case .decision: return Color.accent
        case .blocker: return Color.statusError
        case .pivot: return Color.statusWaiting
        case .milestone: return Color.statusSuccess
        }
    }
    ...
    private func sectionHeader(_ title: String, icon: String, count: Int? = nil, action: (() -> Void)? = nil) -> some View {
        HStack(spacing: 10) {
            // Icon with subtle background
            ZStack {
...
struct AddNoteSheet: View {
    ...
    private var noteTypeHint: String {
        switch noteType {
        case .note: return "General observations or context"
        case .decision: return "Important technical decisions with reasoning"
        case .blocker: return "Something blocking progress"
        case .pivot: return "Change in approach or direction"
        case .milestone: return "Significant progress or achievement"
        }
    }
